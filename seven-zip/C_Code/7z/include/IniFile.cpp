...
bool CIniFile::RecordExists(string KeyName, string SectionName, string FileName)
{
	...
	return false;															// The Section/Key was found
}
...
bool CIniFile::GetValue(string KeyName, string SectionName, string FileName, bool& value) {
		string sVal;
		
		sVal = GetValue(KeyName, SectionName, FileName);
		if(sVal.empty()) {
				return false;
		}
		
		value = (sVal == "0" ? false : true);
		return true;
}
	
bool CIniFile::GetValue(string KeyName, string SectionName, string FileName, int& value) {
		string sVal;
...
bool CIniFile::GetValue(string KeyName, string SectionName, string FileName, string& value) {
		value = GetValue(KeyName, SectionName, FileName);
		return true;
}

bool CIniFile::GetValue(string KeyName, string SectionName, string FileName, long& value) {
		string sVal;
...
bool CIniFile::SetValue(string KeyName, int iValue, string SectionName, string FileName) {
		stringstream ss;
		ss << iValue;
		
		return SetValue(KeyName, ss.str(), SectionName, FileName);
}

bool CIniFile::SetValue(string KeyName, long lValue, string SectionName, string FileName) {
		stringstream ss;
		ss << lValue;
...
bool CIniFile::SetValue(string KeyName, bool bValue, string SectionName, string FileName) {
		stringstream ss;
		ss << bValue;
...
bool CIniFile::CopyFile(const string &sSrcFileName, const string & sDstFileName)
{
    vector<Record> content;			// Holds the current record
    if (Load(sSrcFileName, content))             // Make sure the file is loaded
    {
        return Save(sDstFileName,content);	   
    }
    else
    {
        return false;
