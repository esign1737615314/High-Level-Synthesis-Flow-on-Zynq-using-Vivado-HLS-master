...
void* member_set(void* dst,int val, size_t count)
{
    void* ret = dst;
    while(count--)
    {
...
void sha256_encrypt(unsigned char sUPwd[14], unsigned char digest[32])
{
#pragma HLS INTERFACE s_axilite register port=return bundle=ctrl_bus
#pragma HLS INTERFACE m_axi depth=100 port=digest offset=slave bundle=output
#pragma HLS INTERFACE s_axilite register port=digest bundle=ctrl_bus
#pragma HLS INTERFACE m_axi depth=100 port=sUPwd offset=slave bundle=intput
#pragma HLS INTERFACE s_axilite register port=sUPwd bundle=ctrl_bus
    SHA256_CTX Context;
    sha256_init(&Context);
    ...
    {
        sha256_update(&Context, sUPwd, 14);
